...
# branch_length = 3
HARD_BOX_LIST = [(7, 6), (4, 7), (5, 4), (3, 5), (8, 7), (11, 8), (9, 4), (12, 9), (14, 12), (10, 5), (13, 10),
                 (15, 13)]
...
class BoxWorldRandMAEnv(gym.Env):
    ...
    def set_box(self, box):
        while True:
            ...
            if self.init_world_map[LEFT_BOX_x][LEFT_BOX_y] == 1 and \
                    self.init_world_map[LEFT_BOX_x][LEFT_BOX_y + 1] == 1 and \
                    self.init_world_map[LEFT_BOX_x][LEFT_BOX_y - 1] == 1 and \
                    self.init_world_map[LEFT_BOX_x][LEFT_BOX_y + 2] == 1:
    ...
    def reset(self):
        ...
        agent_init_position = self.np_random.choice(144, 2, replace=False)
        agent_init_x = agent_init_position // 12 + 1
        agent_init_y = agent_init_position % 12 + 1
        self.init_world_map[agent_init_x, agent_init_y] = AGENT_COLOR_IDS
        ...
        assert (self.level in BOX_DICT.keys()), 'BoxWorldRandEnv only supports {} levels'.format(list(BOX_DICT.keys()))
    ...
    def _agent_move(self, agent_id, next_agent_state):
        """
...
if __name__ == '__main__':
    env = BoxWorldRandMAEnv(level='medium')
    # ob = env.observation
    ...
    while True:
        actions = [-1, -1]
        while any(action not in list(range(5)) for action in actions):
            print(f"Actions: {list(enumerate(env.get_action_meanings()))}")
            actions = input("Input action for both agents:")
