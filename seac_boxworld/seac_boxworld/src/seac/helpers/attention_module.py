...
class AttentionModule(nn.Module):
    ...
    def get_att_weights(self, x):
        """Version of forward function that also returns softmax-normalized QK' attention weights"""
        # concatenate all heads' outputs
        A_cat = torch.cat([head(x) for head in self.heads], -1)
        # projecting down to original element size with 2-layer MLP, layer size = entity size
        mlp_out = F.relu(self.linear2(F.relu(self.linear1(A_cat))))
...
class MyTransformerEncoderLayer(nn.TransformerEncoderLayer):
    def __init__(self, d_model, nhead, dim_feedforward=2048, dropout=0.1, activation="relu"):
        ...
        def hook(inner_self, input, output):
            self.atn_weights.append(output[1].detach())

        self.self_attn.register_forward_hook(hook)
    ...
    def forward(
        ...
        out = super(MyTransformerEncoderLayer, self).forward(src, src_mask, src_key_padding_mask)
        weights = self.atn_weights[-1]
        self.atn_weights.clear()
...
class DRRLnet(nn.Module):
    ...
    def forward(self, x):
        ...
        kernelsize = a.shape[
            1]  # but during forward passes called by SummaryWriter, a.shape[1] returns a tensor instead
