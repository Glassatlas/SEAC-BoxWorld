...
class BoxWorldRandEnv(gym.Env):
    ...
    def set_box(self, box):
        while True:
            ...
            if self.init_world_map[LEFT_BOX_x][LEFT_BOX_y] == 1 and self.init_world_map[LEFT_BOX_x][
                LEFT_BOX_y + 1] == 1 and self.init_world_map[LEFT_BOX_x][LEFT_BOX_y - 1] == 1 and \
    ...
    def reset(self):
        ...
        assert (self.level in BOX_DICT.keys()), 'BoxWorldRandEnv only support {} levels'.format(list(BOX_DICT.keys()))
...
if __name__ == '__main__':
    env = BoxWorldRandEnv(level='medium')
    # ob = env.observation
