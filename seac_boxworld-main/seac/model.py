...
from helpers.attention_module import AttentionModule
from helpers.coord_utils import get_coordinate_grid
from helpers.utils import init_flexible
...
class DRRLBase(NNBase):
    ...
    def __init__(
        ...
        if not self.baseline_mode:
            ...
            # how many times the attentional module is to be stacked (weight-sharing -> reuse)
            self.n_att_stack = n_att_stack
            self.attention_module = AttentionModule(conv2w * conv2h, att_elem_size, att_emb_size, n_heads)

            for m in self.attention_module.modules():  # .modules() iterates recursively
            ...
            for i in range(self.n_baseMods):
                ...
                base_dict[f"baseline_conv_{i}_0"] = init_(
                    nn.Conv2d(att_elem_size, att_elem_size, kernel_size=3, stride=1)
                )
                base_dict[f"baseline_batchnorm_{i}_0"] = nn.BatchNorm2d(att_elem_size)
                base_dict[f"baseline_conv_{i}_1"] = init_(
                    nn.Conv2d(att_elem_size, att_elem_size, kernel_size=3, stride=1)
                )
        ...
        # first one has different input size
        fc_dict = OrderedDict([
            ('fc1', init_(nn.Linear(att_elem_size, hidden_size))),
            ('relu1', nn.ReLU())
        ])
    ...
    def forward(self, inputs, rnn_hxs=None, masks=None):
        ...
        batch_size, n_channels, h, w = c.shape
        coordinate_grid = get_coordinate_grid(batch_size, h, w)
        # concat c with positions
        c = torch.cat((c, coordinate_grid), 1)
        if not self.baseline_mode:
            # attentional module
            # careful: we are flattening out x,y dimensions into 1 dimension, so shape changes from (batch_size, #filters,
            # #conv2w, conv2h) to (batch_size, conv2w*conv2h, #filters), because downstream linear layers take last
            # dimension to be input features
            a = c.view(c.shape[0], c.shape[1], -1).transpose(1, 2)
            ...
            for i in range(self.n_baseMods):
                ...
                c = F.relu(c)  # relu
            a = c.view(c.shape[0], c.shape[1], -1).transpose(1, 2)  # flatten (transpose not necessary but we do
        ...
        kernel_size = a.shape[1]
        # of an int. if this causes any trouble it can be replaced by w*h
        if type(kernel_size) == torch.Tensor:
            kernel_size = kernel_size.item()
        pooled = F.max_pool1d(a.transpose(1, 2), kernel_size=kernel_size)  # pool out entity dimension
        # policy module: 4xFC256, then project to logits and value
        p = self.fc_seq(pooled.view(pooled.shape[0], pooled.shape[1]))
    ...
    def get_attention_weights(self, inputs, rnn_hxs=None, masks=None):
        ...
        # batch_maps = torch.cat(batch_size*[self.xymap])
        ...
        att_weights = []
        for i_att in range(self.n_att_stack):
            a, weights = self.attention_module.get_att_weights(a)
